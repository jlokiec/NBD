...
public class ShotDao {
    ...
    private static final String AVG_FOR_LEAGUE = "select avg(sum_shots) from game_results where league=:league allow filtering";
    private static final String AVG_FOR_LEAGUE_SEASON = "select avg(sum_shots) from game_results where league=:league and season=:season allow filtering";
    private static final String SUM_COUNT_FOR_SEASON_HOME_TEAM = "select sum(sum_shots), count(sum_shots) from game_results where season=:season and and home_team=:team allow filtering";
    private static final String SUM_COUNT_FOR_SEASON_AWAY_TEAM = "select sum(sum_shots), count(sum_shots) from game_results where season=:season and away_team=:team allow filtering";
