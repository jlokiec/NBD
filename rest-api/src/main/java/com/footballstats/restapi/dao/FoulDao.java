...
public class FoulDao {
    ...
    private SumAndCount getFoulsSumAndCount(String season, String team, boolean homeTeam) {
        String query = "";
        if (homeTeam) {
            query = "select sum(sum_fouls), count(sum_fouls) from " + TABLE_NAME + " where season=:season and home_team=:team allow filtering";
        } else {
            query = "select sum(sum_fouls), count(sum_fouls) from " + TABLE_NAME + " where season=:season and away_team=:team allow filtering";
        }
