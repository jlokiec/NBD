...
public class RedCardDao {
    ...
    private SumAndCount getRedCardsSumAndCount(String season, String team, boolean homeTeam) {
        String query = "";
        if (homeTeam) {
            query = "select sum(sum_red), count(sum_red) from " + TABLE_NAME + " where season=:season and home_team=:team allow filtering";
        } else {
            query = "select sum(sum_red), count(sum_red) from " + TABLE_NAME + " where season=:season and away_team=:team allow filtering";
        }
