...
public class RedCardDao {
    ...
    private static final String AVG_FOR_LEAGUE = "select avg(sum_red) from game_results where league=:league allow filtering";
    private static final String AVG_FOR_LEAGUE_SEASON = "select avg(sum_red) from game_results where league=:league and season=:season allow filtering";
    private static final String SUM_COUNT_FOR_SEASON_HOME_TEAM = "select sum(sum_red), count(sum_red) from game_results where season=:season and home_team=:team allow filtering";
    private static final String SUM_COUNT_FOR_SEASON_AWAY_TEAM = "select sum(sum_red), count(sum_red) from game_results where season=:season and away_team=:team allow filtering";
    ...
    public double getAvgForSeasonAndTeam(String season, String team) {
        SumAndCount home = getRedCardsSumAndCount(season, team, true);
        SumAndCount away = getRedCardsSumAndCount(season, team, false);
    ...
    private SumAndCount getRedCardsSumAndCount(String season, String team, boolean homeTeam) {
